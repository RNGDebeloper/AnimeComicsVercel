  ...
  return (
    <div id={'sagiri'}>
      <div className={'sagiriWrap'}>
        ...
        /
        <Link className={'sagiri'} href={`/search/a_z?query=${searchParams.query}`}>
          a_z
        </Link>
      </div>
      ...
      <div className={styles.erosNav}>
        ...
        <Link className={styles.navBlue} href={`/search/a_z?query=${searchParams.query}`}>
          a_z
        </Link>
      </div> 
      ...
      {searchParams.page <= 0 && data.length != 18 ? <></>:
        <Pagify p={`/search/a_z?query=${searchParams.query}&`} sagiri={data} page={searchParams.page} />}
    </div>
  )
