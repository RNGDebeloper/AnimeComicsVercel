  ...
  return (
    <div id={'sagiri'}>
      <div className={'sagiriWrap'}>
        ...
        /
        <Link className={'sagiri'} href={`/search/new?query=${searchParams.query}`}>
          new
        </Link>
      </div>
      ...
      <div className={styles.erosNav}>
        ...
        <Link className={styles.navBlue} href={`/search/new?query=${searchParams.query}`}>
          new
        </Link>
      ...
      {searchParams.page <= 0 && data.length != 18 ? <></>:
        <Pagify p={`/search/new?query=${searchParams.query}&`} sagiri={data} page={searchParams.page} />}
    </div>
  )
