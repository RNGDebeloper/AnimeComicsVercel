  ...
  return (
    <div id={'sagiri'}>
      <div className={'sagiriWrap'}>
        ...
        /
        <Link className={'sagiri'} href={`/search/hots?query=${searchParams.query}`}>
          hots
        </Link>
      </div>
      ...
      <div className={styles.erosNav}>
        <Link className={styles.navPop} href={`/search?query=${searchParams.query}`}>
          match
        </Link>
        <Link className={styles.navPop} href={`/search/new?query=${searchParams.query}`}>
          new
        </Link>
        <Link className={styles.navBlue} href={`/search/hots?query=${searchParams.query}`}>
          hots
        </Link>
      ...
      {searchParams.page <= 0 && data.length != 18 ? <></>:
        <Pagify p={`/search/hots?query=${searchParams.query}&`} sagiri={data} page={searchParams.page} />}
    </div>
  )
